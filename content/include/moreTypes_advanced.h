 ...
 * 
 * @param type_name The preprocessor declares a function with this prefix.
 * @param reference_to_type_t (NOT NULL) The variable where the type number will be stored.
 * @param cloneFunction (NOT NULL) A function that creates a copy of the element in dynamic memory allocation.
 * @param deleteFunction (NOT NULL) A function that frees the memory allocated by the element.
 * @param compareFunction A function that must return `0` if element1 == element2, `number > 0` if element1 > element2 or `number < 0` if element1 < element2.
 * @param hashFunction A function that turns the element into an integer. NULL hash function means a non-hashable element.
 * 
 * @note The `type_t` variable must be global.
 */
